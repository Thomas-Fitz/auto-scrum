...
3. Find ALL real issues across these dimensions:
   ...
   - **Code quality** — clean, maintainable, no dead code. Specifically flag: (a) a member field/flag/variable that is WRITTEN but never READ — dead code, and HIGH if it gates an AC (the classic "looks wired but isn't" bug); (b) a helper / fixture body duplicated across ≥2 files — must be extracted to a shared module, not copy-pasted (duplicated definitions drift silently) — REJECT until extracted.
